      ...
      minPoint{p0}, maxPoint{p1}
{
    ...
    sides.add(std::make_shared<FlipFace>(
        std::make_shared<AARect<utils::Axis::Z>>(p0.x(), p1.x(), p0.y(), p1.y(), p0.z(), material)));

    sides.add(std::make_shared<AARect<utils::Axis::Y>>(p0.x(), p1.x(), p0.z(), p1.z(), p1.y(), material));
    sides.add(std::make_shared<FlipFace>(
        std::make_shared<AARect<utils::Axis::Y>>(p0.x(), p1.x(), p0.z(), p1.z(), p0.y(), material)));
...
Hittable::HitType Box::getCollisionData(const Ray &ray, HitRecord &record, WeightedPdf &pdf,
                             double tMin, double tMax, bool flip) const
{
    return sides.getCollisionData(ray, record, pdf, tMin, tMax, flip);
}